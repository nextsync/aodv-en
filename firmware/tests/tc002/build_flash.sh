...
usage() {
  cat <<'EOF'
uso:
  zsh firmware/tests/tc002/build_flash.sh node_a <PORTA> <TARGET_MAC_NODE_C>
  zsh firmware/tests/tc002/build_flash.sh node_b <PORTA>
  zsh firmware/tests/tc002/build_flash.sh node_c <PORTA>

exemplos:
  zsh firmware/tests/tc002/build_flash.sh node_a /dev/ttyUSB0 28:05:A5:33:AA:CC
  zsh firmware/tests/tc002/build_flash.sh node_b /dev/ttyUSB1
  zsh firmware/tests/tc002/build_flash.sh node_c /dev/ttyUSB2

observacao:
  para node_a, o TARGET_MAC deve ser o MAC do NODE_C (nunca o proprio MAC do NODE_A)
EOF
}
...
if [[ "$ROLE" == "node_a" ]]; then
  if [[ -z "$TARGET_MAC" ]]; then
    echo "node_a exige TARGET_MAC_NODE_C" >&2
    usage
    exit 1
  fi
...
BUILD_DIR="$FW_DIR/build/tc002_${ROLE}"
