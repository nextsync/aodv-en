...
usage() {
  cat <<'EOF'
uso:
  zsh firmware/tests/tc001/build_flash.sh node_a <PORTA> <TARGET_MAC_NODE_B>
  zsh firmware/tests/tc001/build_flash.sh node_b <PORTA>

exemplos:
  zsh firmware/tests/tc001/build_flash.sh node_a /dev/ttyUSB0 28:05:A5:33:EB:80
  zsh firmware/tests/tc001/build_flash.sh node_b /dev/ttyUSB1

observacao:
  para node_a, o TARGET_MAC deve ser o MAC do NODE_B (nunca o proprio MAC do NODE_A)
EOF
}
...
case "$ROLE" in
  node_a|node_b)
    ;;
  *)
    echo "papel invalido: $ROLE (use node_a ou node_b)" >&2
    exit 1
    ;;
esac
...
if [[ "$ROLE" == "node_a" ]]; then
  if [[ -z "$TARGET_MAC" ]]; then
    echo "node_a exige TARGET_MAC_NODE_B" >&2
    usage
    exit 1
  fi
...
BUILD_DIR="$FW_DIR/build/tc001_${ROLE}"
