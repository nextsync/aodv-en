...
composto por três módulos ESP32 DevKit V1 disponíveis no período de
coleta, alimentados por fontes USB 5V --- a disponibilidade de hardware
limitou a campanha a três nós, com os demais cenários avaliados por
simulação. Cada nó executou o mesmo firmware, configurado com
identificador único por meio de variáveis de compilação.
...
planejamento detalhado na Seção~\ref{planejamento-experimental}, com execução parcial em hardware
--- seis repetições no cenário C1-3n (três nós em concentrador) --- e
complementação por simulação para os cenários de maior escala e
diâmetro. As métricas foram coletadas automaticamente pelo firmware e
...
Por limitação de disponibilidade de hardware no período de coleta, apenas
uma versão reduzida do cenário C1 --- denominada C1-3n, com três nós em
concentrador e seis repetições por algoritmo --- foi executada em
hardware real; os demais cenários foram avaliados por simulação
...
retransmissões indefinidas. O Quadro~\ref{quad:parametros} apresenta os parâmetros adotados para cada algoritmo.
...
das transmissões sobre a autonomia dos dispositivos. O Quadro~\ref{quad:metricas} sintetiza as métricas, suas definições e fórmulas.
