...
Adotam-se, em todo este capítulo, três convenções de medida. A \emph{taxa de
entrega} é calculada apenas sobre as repetições com telemetria de entrega do
nó de destino (as poucas repetições sem esse reporte são excluídas dessa
métrica, e não substituídas por outra). As métricas de custo \emph{por pacote
entregue} (transmissões, recepções e energia) são razões agregadas --- total
da rede dividido pelo total de pacotes entregues ---, motivo pelo qual
aparecem como ponto único. Os intervalos de confiança e coeficientes de
variação usam desvio padrão amostral ($n-1$).
...
Latência unidirecional (ms) & 45,9 $\pm$ 3,5 & 31,6 $\pm$ 1,7 \\
NRL (controle de rota/entregue) & 18,0 $\pm$ 4,7 & 0,0 \\
Transmissões por entrega & 54,2 & 68,8 \\
Energia agregada (J, estimada) & 39,4 $\pm$ 0,64 & 28,4 $\pm$ 0,03 \\
\textbf{Energia por pacote entregue (J)} & \textbf{0,69} & \textbf{3,75} \\
\bottomrule
\end{tabular}

\vspace{2pt}
{\footnotesize Fonte: Elaborado pelos autores. O PDR de entrega é a média entre as
repetições com telemetria de entrega do destino (AODV-EN $n=30$; \emph{flooding} $n=27$,
três repetições sem reporte do destino foram excluídas desta métrica). \emph{Transmissões}
e \emph{energia por pacote entregue} são razões agregadas (total da rede $\div$ total
entregue), por isso reportadas como ponto único, sem IC.}
\end{table}
...
\begin{table}[H]
\centering
\caption{Significância estatística das diferenças AODV-EN vs.~Flooding (cenário C1; AODV-EN $n=30$; \emph{flooding} $n=30$, exceto PDR e transmissões por entrega, com $n=27$ repetições de entrega válida, e latência, com $n=29$)}\label{tab:stats}
\begin{tabular}{@{}lcccl@{}}
\toprule
Métrica & $t$ (Welch) & $p$ & $d$ de Cohen & Efeito \\
\midrule
PDR de entrega & 42,3 & $<0{,}001$ & 11,6 & altíssimo \\
Latência unidirecional & 7,2 & $<0{,}001$ & 1,85 & grande \\
...
\vspace{2pt}
{\footnotesize Fonte: Elaborado pelos autores. Teste de Mann--Whitney concordante ($p<0{,}05$)
em todas as métricas. A linha \emph{Transmissões por entrega} testa a razão por repetição
(transmissões/entregues), cuja média entre repetições (102,9) difere da razão agregada
adotada como referência na Tabela~\ref{tab:comparacao} (68,8 = total transmitido $\div$ total
entregue).}
\end{table}
...
de efeito da entrega ($d = 11{,}6$) e do consumo estimado ($d = 8{,}65$)
...
\emph{flooding}, em contraste, variou de 1,7\% a 36,2\% (mediana 11,5\%;
$\mathrm{CV} = 65{,}5\%$), com 17 das 27 repetições abaixo de 15\%. A
diferença de variabilidade --- CV de 4\%
contra 65\% --- mostra que o AODV-EN não apenas entrega mais, mas o faz de
...
saltos), o AODV-EN entregou 94,1\% dos pacotes contra apenas 13,9\% do
...
\textbf{Latência unidirecional.} A latência unidirecional (\emph{one-way}) do \emph{flooding}
(31,6 ms) foi menor que a do AODV-EN (45,9 ms), mas o número não
representa vantagem: trata-se de \emph{viés de sobrevivência}
(\emph{survivorship bias}). No \emph{flooding}
...
entregue, dado pela razão entre as transmissões agregadas na rede e os
pacotes entregues: 54,2 para o AODV-EN contra 68,8 para o \emph{flooding} ---
isto é, mesmo somando todo o controle de rota, o AODV-EN custa cerca de 20\%
menos transmissões por pacote efetivamente entregue (o \emph{flooding}
transmite cerca de 1,3 vez mais por entrega útil). Esse valor do
...
maior no AODV-EN (39,4 J) que no \emph{flooding} (28,4 J) na janela. Esse
número \emph{bruto}, porém, induz a uma conclusão equivocada por duas razões.
Primeiro, a comparação não está em pé de igualdade: o AODV-EN manteve nove nós
ativos com HELLO periódico e encaminhamento, enquanto o \emph{flooding} teve
apenas sete nós contabilizados e não emite HELLO. Segundo, e sobretudo, o
número bruto ignora que o AODV-EN entrega muito mais pacotes. A métrica justa é
a \emph{energia por pacote efetivamente entregue}, obtida pela razão entre a
energia total da rede e o total de pacotes entregues: 0,69~J/pacote para o
AODV-EN contra 3,75~J/pacote para o \emph{flooding} --- ou seja, o AODV-EN é
cerca de \textbf{cinco vezes mais eficiente em energia por entrega útil}. O
\emph{flooding} gasta energia disseminando pacotes que, em sua maioria, não
sobrevivem ao multi-hop. Cabe
...
Latência unidirecional (ms) & 34,9 $\pm$ 2,7 & 20,5 $\pm$ 0,6 \\
...
\vspace{2pt}
{\footnotesize Fonte: Elaborado pelos autores. \emph{Transmissões}, \emph{recepções} e
\emph{energia por pacote entregue} são razões agregadas (total da rede $\div$ total
entregue), por isso reportadas como ponto único; \emph{recepções por entrega} indica a
ocupação de canal. As diferenças de PDR, PDR de confirmação, latência e NRL são
significativas (teste $t$ de Welch, $p<0{,}001$).}
\end{table}
...
caminhos eleva sua entrega a 77,8\%, contra apenas 13,9\% no C1. O AODV-EN
ainda entrega significativamente mais (91,4\% contra 77,8\%; $t = 6{,}19$,
$p < 0{,}001$, $d = 1{,}6$), mas a sua vantagem de confiabilidade
\emph{encolhe} --- de cerca de 80 pontos percentuais no C1 para 13 pontos no
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={PDR do AODV-EN e do flooding nos cenarios C1 (cadeia) e C3 (malha): na malha o flooding recupera e o gap encolhe.}]{figuras/fig_topology_dependence.png}}
...
de canal}: cada pacote entregue pelo \emph{flooding} custa 84,6 recepções
agregadas na rede, contra 55,5 do AODV-EN --- consequência da tempestade de
...
0,81~J). A menor latência do \emph{flooding} (20,5 contra 34,9~ms) reflete,
novamente, \emph{viés de sobrevivência} e o uso do caminho de difusão mais curto.
Em síntese, mesmo onde o \emph{flooding} entrega bem, o AODV-EN o supera em
entrega e em custo de canal por pacote útil.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={AODV-EN vs flooding no cenario C3: PDR, latencia e recepcoes por entrega.}]{figuras/fig_c3_compare.png}}
...
Quanto à estabilidade entre repetições, o AODV-EN na malha foi consistente
(entrega de 76,7\% a 100\%; mediana 93,3\%; coeficiente de variação de 6,7\%),
com 26 das 30 repetições iguais ou acima de 85\%. O \emph{flooding} apresentou maior
dispersão (48,3\% a 95,0\%; mediana 78,5\%; CV de 13,2\%), refletindo a
sensibilidade da inundação às colisões em cada repetição. A
Figura~\ref{fig:c3-pdr-rep} mostra a entrega por repetição para os dois
algoritmos.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Taxa de entrega por repeticao no cenario C3 para AODV-EN e flooding.}]{figuras/fig_c3_pdr_rep.png}}
\caption{Taxa de entrega por repetição no cenário C3 (malha densa): o AODV-EN é
mais estável (CV 6,7\%) que o \emph{flooding} (CV 13,2\%).}\label{fig:c3-pdr-rep}
\end{figure}

À semelhança do cenário C1, as diferenças do C3 foram submetidas ao teste $t$
de Welch e ao $d$ de Cohen (Tabela~\ref{tab:c3-stats}). Mesmo na malha densa,
em que o \emph{flooding} não colapsa, as vantagens do AODV-EN em entrega,
confirmação e latência permanecem estatisticamente significativas, com
tamanhos de efeito de grandes a altíssimos.

\begin{table}[H]
\centering
\caption{Significância estatística das diferenças AODV-EN vs.~Flooding (cenário C3, $n=30$ por grupo)}\label{tab:c3-stats}
\begin{tabular}{@{}lcccl@{}}
\toprule
Métrica & $t$ (Welch) & $p$ & $d$ de Cohen & Efeito \\
\midrule
PDR de entrega & 6,2 & $<0{,}001$ & 1,60 & grande \\
PDR confirmação ida-volta & 14,1 & $<0{,}001$ & 3,64 & altíssimo \\
Latência unidirecional & 10,1 & $<0{,}001$ & 2,61 & altíssimo \\
\bottomrule
\end{tabular}

\vspace{2pt}
{\footnotesize Fonte: Elaborado pelos autores. Teste de Mann--Whitney concordante
($p<0{,}05$). A latência menor do \emph{flooding} reflete viés de sobrevivência
(Seção~\ref{discussuxe3o-por-muxe9trica}); a NRL não é testável (nula no \emph{flooding}) e
as métricas de custo por entrega são razões agregadas.}
\end{table}
...
de um relé \emph{on-path}, que dispõe de um relé alternativo.

A falha é
induzida por \emph{firmware}: o relé do caminho principal suprime todas as suas
transmissões (HELLO, RREP, RERR e encaminhamento de dados) por 15~s a cada 45~s
(33\% de indisponibilidade), simulando a queda e o
retorno cíclicos do nó; os vizinhos detectam a quebra pela ausência das mensagens HELLO,
...
Latência unidirecional (ms) & 60,7 $\pm$ 14,6 \\
...
\vspace{2pt}
{\footnotesize Fonte: Elaborado pelos autores. PDR de entrega sobre as 26 repetições com
telemetria de entrega do destino (quatro sem reporte foram excluídas dessa métrica).
\emph{Transmissões} e \emph{energia por pacote entregue} são razões agregadas. NRL e
energia não são diretamente comparáveis às do C1 por diferirem o número de nós (6 vs.~10)
e a topologia.}
\end{table}

\textbf{Impacto da falha.} Em relação ao mesmo protocolo \emph{sem} falha
(cenário C1, 94,1\%), a taxa de entrega caiu para 70,6\% --- uma redução de
cerca de 24 pontos percentuais, estatisticamente significativa (teste $t$ de
Welch: $t = 4{,}5$, $p < 0{,}001$; $d$ de Cohen $= 1{,}28$, efeito grande). A
latência subiu de 45,9~ms para 60,7~ms ($t = -1{,}94$, $p \approx 0{,}06$,
diferença apenas marginalmente não significativa),
refletindo o tempo de detecção da quebra (\emph{timeout} de HELLO) somado à
re-descoberta de rota. A confirmação fim-a-fim foi a métrica mais penalizada,
caindo de 91,7\% para 47,6\% ($t = 7{,}1$, $p < 0{,}001$, $d = 1{,}83$), pois o
ACK percorre duas vezes o caminho sujeito à queda. A
Tabela~\ref{tab:c4-stats} consolida os testes e a Figura~\ref{fig:c4-metrics}
contrasta as duas condições.

\begin{table}[H]
\centering
\caption{Significância estatística do impacto da falha: C1 (sem falha) vs.~C4 (sob falha)}\label{tab:c4-stats}
\begin{tabular}{@{}lcccl@{}}
\toprule
Métrica & $t$ (Welch) & $p$ & $d$ de Cohen & Efeito \\
\midrule
PDR de entrega & 4,5 & $<0{,}001$ & 1,28 & grande \\
PDR confirmação ida-volta & 7,1 & $<0{,}001$ & 1,83 & grande \\
Latência unidirecional & $-1{,}9$ & $\approx 0{,}06$ & $-0{,}51$ & médio \\
\bottomrule
\end{tabular}

\vspace{2pt}
{\footnotesize Fonte: Elaborado pelos autores. A diferença de latência é apenas
marginalmente não significativa (os dois testes cercam $\alpha = 0{,}05$). PDR de
entrega com $n = 26$ repetições de entrega válida no C4.}
\end{table}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Comparacao do PDR e da latencia do AODV-EN entre o cenario C1 (sem falha) e o C4 (sob falha), com barras de IC95.}]{figuras/fig_c4_metrics.png}}
...
\textbf{Distribuição e variabilidade.} A entrega no C4 variou de 9,8\% a 100\%
entre as repetições (mediana 77,0\%; coeficiente de variação de 38\%): 12 das
26 repetições preservaram entrega $\geq 80\%$, enquanto 4 ficaram abaixo de
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Taxa de entrega por repeticao no cenario C4, evidenciando a variabilidade da recuperacao sob falha.}]{figuras/fig_c4_pdr_rep.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Entregas acumuladas ao longo do tempo em uma repeticao do cenario C4: patamares coincidem com a falha do rele e a retomada evidencia a recuperacao.}]{figuras/fig_c4_selfheal.png}}
...
Figura~\ref{fig:sim-crossover} apresenta o resultado: sob enlaces ideais e um
TTL suficiente para o diâmetro da grade, ambos os algoritmos entregam a
totalidade dos pacotes, mas a razão transmissões-por-entrega não é monotônica
nas grades pequenas (o AODV-EN é mais econômico já em 4 nós, o \emph{flooding}
volta a sê-lo por volta de 9 nós); a partir de cerca de onze nós, contudo, o
roteamento do AODV-EN passa a custar consistentemente menos transmissões por
entrega, com a diferença ampliando-se à medida que a rede cresce. Além disso, por depender de um
TTL fixo, o \emph{flooding} tem o alcance estruturalmente limitado ao número de
saltos configurado --- destinos além desse teto tornam-se inalcançáveis ---,
enquanto o AODV-EN, por manter rotas, não está sujeito a esse limite. A
simulação assume enlaces ideais
...
multi-hop de dez nós em hardware, o AODV-EN entrega 6,8 vezes mais
pacotes com cerca de 20\% menos transmissões por entrega; a sua vantagem
...
invalidação por falha de enlace ---, com disseminação de RREQ por
\emph{broadcast} e seleção de rota pela métrica híbrida (contagem de saltos
combinada com penalização por RSSI; pesos padrão de salto~8 e RSSI~1),
efetivamente ativa no \emph{firmware} flashado na campanha. A política de
gerenciamento de \emph{peers} por substituição LRU também está implementada
(Capítulo~\ref{cap:implementacao}). Nenhuma das duas, contudo, foi objeto de
um experimento de isolamento: o cenário C1 multi-hop já aproxima o coletor do
limite do \emph{cache} de \emph{peers} e exercita a seleção de rota sobre
múltiplos saltos, mas a quantificação separada do ganho da métrica híbrida
sobre a contagem de saltos pura e do efeito da substituição por LRU permanece
como objeto de trabalhos futuros, em topologias mais densas e com variação
deliberada de RSSI por enlace.

As principais limitações do estudo são quatro. Primeiro, a cobertura de
cenários: a campanha em hardware abrange C1 (cadeia de dez nós), C3 (malha
densa de dez nós, cinco saltos) e C4 (falha induzida, seis nós), todos com
trinta repetições por algoritmo, ficando a avaliação quantitativa do
\emph{flooding} sob falha (C4) e o cenário C2 (árvore hierárquica) como
trabalho futuro sobre a mesma infraestrutura. Segundo, o caráter estimado do
consumo energético, calculado a partir do perfil de \emph{datasheet} do ESP32
e não medido com instrumentação física --- a medição direta de corrente (por
exemplo, com um sensor INA219 em série com a alimentação) fica registrada como
trabalho futuro. Terceiro, as campanhas do AODV-EN e do \emph{flooding} foram
coletadas em sequência, com reprogramação e nova distribuição física dos nós
entre elas, o que introduz pequena variação de topologia e de condições de
canal. Por fim, parte da telemetria de rede não retornou ao coletor: na
campanha de \emph{flooding}, apenas sete dos dez nós reportaram, subestimando
as métricas de custo agregado desse algoritmo, e algumas repetições (três do
\emph{flooding} em C1, quatro em C4) não tiveram telemetria de entrega do
destino, sendo excluídas do cálculo do PDR de entrega. A medição física de
energia, a execução do cenário C2 (árvore) e a avaliação do \emph{flooding}
sob falha constituem desdobramentos diretos para trabalhos futuros.
