...
de maior sobrecarga (\emph{overhead}) de controle. Os protocolos reativos, como o
...
saltos sobre ESP-NOW com alta taxa de entrega --- \emph{Packet Delivery
Ratio} de entrega elevado e estável entre repetições --- e custo de
canal, medido em transmissões por pacote entregue, inferior ao da
inundação controlada. Espera-se, ademais, que essa vantagem do roteamento
se acentue no regime multi-hop, no qual a inundação tende a degradar por
colisão e por ausência de retransmissão fim-a-fim, à medida que a rede
cresce em tamanho e diâmetro. Essas afirmações são avaliadas
quantitativamente no Capítulo~\ref{cap:resultados} por meio de testes de
