...
Uma das principais decisões de
...
mensagens ACK em nível de aplicação.

\subsection{Gerenciamento Dinâmico de Peers}\label{gerenciamento-dinamico-de-peers}

O ESP-NOW impõe restrições quanto ao número de peers que podem ser
...
com uma penalidade associada à qualidade do sinal recebido. A formulação geral da métrica é dada pela Equação~\ref{eq:custo_impl}:

\begin{equation}\label{eq:custo_impl}
  \text{Custo}(\text{rota}) = \alpha \times \text{HopCount} + \beta \times \text{Penalidade}(\overline{\text{RSSI}})
\end{equation}

\noindent onde $\alpha = 8$ e $\beta = 1$ são os pesos padrão (configuráveis por
\texttt{AODV\_EN\_ROUTE\_METRIC\_HOP\_WEIGHT} e
\texttt{AODV\_EN\_ROUTE\_METRIC\_RSSI\_WEIGHT}). A função
$\text{Penalidade}(\overline{\text{RSSI}})$ retorna um valor inteiro
no intervalo $[0, 100]$, conforme a Equação~\ref{eq:penalidade_impl}:

\begin{equation}\label{eq:penalidade_impl}
  \text{Penalidade}(\overline{\text{RSSI}}) =
  \operatorname{clamp}\!\left(
    \frac{(\text{RSSI}_\text{best} - \overline{\text{RSSI}}) \times 100}
         {\text{RSSI}_\text{best} - \text{RSSI}_\text{worst}},\;0,\;100
...
\noindent com $\text{RSSI}_\text{best} = -55\,\text{dBm}$ e
$\text{RSSI}_\text{worst} = -90\,\text{dBm}$. O valor $\overline{\text{RSSI}}$
é atualizado por média móvel exponencial (EWMA, fator~$3/4$) a cada
mensagem recebida do vizinho. Penalidade~$= 0$ indica sinal ótimo
(acima de $-55\,\text{dBm}$); Penalidade~$= 100$ indica sinal mínimo
(abaixo de $-90\,\text{dBm}$). Quanto menor o número de saltos e melhor
o sinal, menor o custo da rota. Essa adaptação permite que a seleção de
