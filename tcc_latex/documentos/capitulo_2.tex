...
útil na versão original (v1), com um \emph{overhead} de cabeçalho de
aproximadamente 43 bytes; a versão~2 (v2), disponível a partir do ESP-IDF~v5.x e
adotada neste trabalho, eleva esse limite para cerca de 1490 bytes por quadro. O
...
Mecanismo de acesso ao meio & CSMA p-persistent (slot $\sim$481
µs) \\
...
\subsection{Confirmação de enlace e retransmissões}\label{confirmacao-de-enlace}

Um aspecto determinante para o comportamento dos algoritmos avaliados é a
assimetria de confiabilidade entre os modos \emph{unicast} e
\emph{broadcast} do ESP-NOW. O protocolo opera sobre a camada de enlace
802.11 por meio de quadros de ação (\emph{action frames}) de fabricante,
sujeitos ao acesso ao meio por CSMA/CA. No modo \emph{unicast} (destino
endereçado), o quadro é confirmado por um ACK de hardware e
retransmitido automaticamente em caso de falha, até um limite de
tentativas; cada retransmissão acrescenta latência da ordem do tempo de
\emph{slot} e do \emph{backoff}, o que explica a faixa observada na
literatura --- de $\sim$2,8 ms na primeira tentativa a dezenas de
milissegundos sob múltiplas retransmissões \cite{becker2025}. No modo
\emph{broadcast} (endereço FF:FF:FF:FF:FF:FF), ao contrário, \emph{não há
ACK nem retransmissão}: cada quadro é transmitido uma única vez, e a sua
perda por colisão ou enlace fraco é definitiva.

Essa diferença tem consequência direta sobre roteamento. Uma abordagem por
inundação (\emph{flooding}), que dissemina dados por \emph{broadcast}, não
dispõe de recuperação de enlace e degrada rapidamente em múltiplos saltos,
pois a probabilidade de sobrevivência do pacote decai com o produto das
probabilidades por salto. Já um protocolo de roteamento que encaminha
dados por \emph{unicast} ao longo de uma rota estabelecida aproveita a
confirmação e a retransmissão automáticas do enlace, recuperando perdas
salto a salto. Essa assimetria é central para interpretar os resultados
experimentais (Capítulo~\ref{cap:resultados}).
...
extensões futuras. Estratégias de comunicação multi-hop para malhas IoT
de baixa potência também têm sido investigadas de forma mais ampla
\cite{arregui2025}, e levantamentos recentes sistematizam os protocolos e
desafios de redes mesh sem fio \cite{chaizeng2021}.

O Quadro~\ref{quadro:correlatos} consolida as soluções de mesh para ESP32
e as posiciona frente à proposta deste trabalho, evidenciando a lacuna que
o AODV-EN preenche: ser a única abordagem sobre ESP-NOW fundamentada em um
protocolo de roteamento padronizado.

\begin{quadro}[H]
\centering
\caption{Soluções de rede mesh para ESP32 e posicionamento do AODV-EN}\label{quadro:correlatos}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.27\linewidth}>{\raggedright\arraybackslash}p{0.27\linewidth}cc>{\raggedright\arraybackslash}p{0.14\linewidth}@{}}
\toprule
Solução & Base de roteamento & Padron. & Multi-hop & Consumo \\
\midrule
ESP-WIFI-MESH \cite{sestak2022} & Árvore proprietária sobre Wi-Fi & Não & Sim & Alto \\
PainlessMesh \cite{sestak2022} & Árvore proprietária sobre Wi-Fi & Não & Sim & Alto \\
BRAM-NOW \cite{cujilema2023} & Mesh \emph{ad hoc} sobre ESP-NOW & Não & Sim & Baixo \\
ESP-NOW ponto-a-ponto \cite{becker2025} & Sem roteamento & --- & Não & Baixo \\
Estratégias multi-hop IoT \cite{arregui2025} & Heurísticas multi-hop & Não & Sim & Baixo \\
\textbf{AODV-EN (este trabalho)} & AODV reativo (RFC 3561) & \textbf{Sim} & Sim & Baixo \\
\bottomrule
\end{tabular}

\vspace{2pt}
{\footnotesize Fonte: Elaborado pelos autores. ``Consumo'' refere-se ao custo do plano de manutenção da malha: soluções sobre Wi-Fi mantêm o rádio ativo continuamente, enquanto as baseadas em ESP-NOW dispensam associação e \emph{beaconing}.}
\end{quadro}
...
A título de exemplo, considere uma cadeia de três nós $A$, $B$ e $C$, na
qual $A$ deseja enviar dados a $C$ e só alcança diretamente $B$. (1) $A$
incrementa seu número de sequência e difunde um RREQ para $C$. (2) $B$
recebe o RREQ, registra uma rota reversa para $A$ (próximo salto $A$,
1~salto), incrementa o contador de saltos e, não sendo o destino nem
possuindo rota para $C$, retransmite o RREQ. (3) $C$ recebe o RREQ,
registra a rota reversa para $A$ via $B$ (2~saltos) e, sendo o destino,
responde com um RREP em \emph{unicast} para $B$. (4) $B$ recebe o RREP,
cria a rota direta para $C$ (próximo salto $C$, 1~salto) e o encaminha a
$A$. (5) $A$ recebe o RREP e registra a rota para $C$ via $B$ (2~saltos):
a rota está estabelecida e os dados passam a ser encaminhados por
\emph{unicast} $A \rightarrow B \rightarrow C$. Os números de sequência
garantem que, havendo múltiplas respostas, $A$ adote sempre a rota com
informação mais recente, evitando laços.
...
\section{Segurança em Roteamento Ad-Hoc}\label{seguranca-roteamento-ad-hoc}

O AODV, conforme especificado na RFC 3561 \cite{perkins2003}, não
incorpora mecanismos de segurança: assume que todos os nós são honestos e
não autentica as mensagens de controle. Essa premissa, herdada por
qualquer adaptação direta do protocolo, expõe a malha a um conjunto de
ataques conhecidos na literatura de redes \emph{ad hoc}:

\begin{itemize}
\item \textbf{Buraco negro (\emph{black hole}).} Um nó malicioso responde
  a toda RREQ anunciando-se como a melhor rota (número de sequência alto,
  poucos saltos) e, ao atrair o tráfego, descarta os dados em vez de
  encaminhá-los.
\item \textbf{RERR forjado.} Um adversário injeta mensagens de erro de
  rota (RERR) falsas, invalidando rotas válidas e forçando descobertas
  repetidas, o que degrada a disponibilidade e amplia o \emph{overhead} de
  controle.
\item \textbf{Inundação de RREQ.} O envio massivo de requisições de rota
  esgota banda e energia dos nós, caracterizando uma negação de serviço.
\item \textbf{Sybil e \emph{spoofing} de identidade.} Como o endereçamento
  é por MAC e não há autenticação, um nó pode assumir múltiplas
  identidades ou personificar outro, corrompendo as tabelas de rota.
\end{itemize}

No caso específico do ESP-NOW, a difusão de enlace por \emph{broadcast}
ocorre, por padrão, sem criptografia, de modo que qualquer dispositivo no
mesmo canal e identificador de rede pode escutar e injetar quadros. A
literatura propõe extensões seguras do AODV (por exemplo, com assinatura
de mensagens e numeração protegida), ao custo de gerência de chaves e
\emph{overhead} computacional --- particularmente sensível em
microcontroladores. Neste trabalho, o AODV-EN é avaliado quanto a
desempenho de roteamento sob a premissa de nós honestos; o endurecimento
do protocolo contra os ataques acima, e a avaliação do custo associado em
ESP32, constituem trabalho futuro (Capítulo~\ref{cap:conclusao}).
