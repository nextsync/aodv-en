...
1991 sob o conceito de Computação Ubíqua, materializa-se atualmente em
...
luminosidade e transmitir os dados coletados através de enlaces sem fio
...
Quadro 1 sintetiza os principais requisitos e suas implicações para o
projeto de redes de sensores.

\textbf{Quadro 1 -- Requisitos críticos para projeto de Redes de
Sensores Sem Fio}
...
\textbf{Quadro 2 -- Especificações técnicas do microcontrolador ESP32}
...
energético. O Quadro 3 apresenta os modos de energia disponíveis e seus
respectivos consumos típicos, conforme especificado no \emph{datasheet}
do fabricante.

\textbf{Quadro 3 -- Modos de operação e consumo energético do ESP32}
...
brasileiro, democratiza o acesso à tecnologia e viabiliza a construção
...
\textbf{Quadro 4 -- Características técnicas do protocolo ESP-NOW}
...
Número máximo de peers & 20 peers (com criptografia) / 10 peers (sem
criptografia) \\
\end{longtable}
}

Fonte: Becker et al.~(2025); \citeonline{espressif2024}.
...
multi-hop. O protocolo foi concebido exclusivamente para comunicações
ponto-a-ponto ou em topologia estrela, não oferecendo mecanismos para
encaminhamento de pacotes através de nós intermediários
\cite{becker2025}. A segunda limitação refere-se ao número máximo de
\emph{peers} simultâneos, cada dispositivo ESP32 pode manter no máximo
20 \emph{peers} registrados em sua tabela de pares quando utilizada
criptografia, ou 10 \emph{peers} sem criptografia. Essa restrição,
imposta por limitações de memória do hardware, impede a escalabilidade
direta da rede e exige estratégias de gerenciamento dinâmico de
\emph{peers} para contorná-la. A terceira limitação relaciona-se à
...
abordagens, mas introduz complexidade adicional na implementação.

\textbf{Quadro 5 -- Comparativo entre categorias de protocolos de
roteamento ad-hoc}
...
que influenciam o comportamento e desempenho do protocolo. O Quadro 6
...
\textbf{Quadro 6 -- Parâmetros de configuração do AODV (RFC 3561)}
...
\section{Considerações para o Projeto do
AODV-EN}\label{considerauxe7uxf5es-para-o-projeto-do-aodv-en}

A adaptação do protocolo AODV para operação sobre ESP-NOW (AODV-EN)
requer a consideração de três aspectos fundamentais: gerenciamento do
...
O limite de 20 \emph{peers} simultâneos imposto pelo ESP-NOW exige uma
estratégia de gerenciamento dinâmico para permitir a comunicação com um
número potencialmente maior de nós ao longo do tempo. A política LRU
(\emph{Least Recently Used}) é proposta como mecanismo de substituição:
quando a tabela de \emph{peers} está cheia e uma nova comunicação é
necessária, o \emph{peer} menos recentemente utilizado é removido para
dar lugar ao novo. Essa abordagem baseia-se no princípio de localidade
temporal: nós comunicados recentemente têm maior probabilidade de serem
comunicados novamente.
...
Strength Indicator}--RSSI). Considerando essa perspectiva, o AODV-EN
fundamenta-se em uma métrica composta que integra o número de saltos e a
qualidade média dos enlaces ao longo da rota, podendo ser expressa por:

\textbf{\emph{Custo(rota) = α × HopCount + β × (1 / RSSI\_médio)}}

Onde α e β são pesos configuráveis que permitem balancear a importância
relativa da distância lógica (número de saltos) e da qualidade dos
enlaces. Essa abordagem possibilita a seleção de rotas que otimizem
...
baseados no perfil de consumo dos dispositivos empregados. De modo
geral, a energia consumida durante a transmissão de um pacote pode ser
expressa como o produto entre a potência de transmissão e o tempo de
envio, conforme:

\textbf{\emph{E\_tx = P\_tx × t\_tx = (V × I\_tx) × t\_tx}}

Onde que \emph{E\_tx} representa a energia consumida por transmissão,
\emph{P\_tx} a potência elétrica durante o envio, \emph{V} a tensão de
operação e \emph{I\_tx} a corrente elétrica associada ao modo de
...
roteamento sob condições semelhantes de operação.
